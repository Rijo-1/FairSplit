...
async def extract_bill_base64(request: Base64ImageRequest):
    request_id = str(uuid.uuid4())
    import base64

    logger.info(
        "Bill extraction base64 upload received: filename=%s content_type=%s request_id=%s",
        request.filename,
        request.mime_type,
        request_id,
    )
    ...
    try:
        ...
        logger.error("Base64 decoding failed: %s", e, extra={"request_id": request_id})
        raise HTTPException(
            status_code=400,
            detail={
                "error": {
    ...
    extractor = BillExtractor()
    try:
        bill = await extractor.extract_from_image(contents, request.mime_type)
        logger.info("Bill extracted successfully", extra={"request_id": request_id, "items": len(bill.items)})
        return {"bill": bill, "request_id": request_id}
    except BillExtractionError as e:
        logger.warning("Bill extraction failed: %s", e.message, extra={"request_id": request_id})
